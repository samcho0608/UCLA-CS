  ...
  while(*a != ' ' && *b != ' ')
    {
      if(f_option)
      {
        a_val = toupper((unsigned char)((*a) ^ 42));
        b_val = toupper((unsigned char)((*b) ^ 42));
      }
      else
      {
        a_val = (*a) ^ 42;
...
int main(int argc, char** argv)
{
  ...
  // if reg file, allocate all memory from the beginning
  if(file_size != 0)
  {
  ...
  // otherwise, approach it byte by byte
  else
  {
